...
export class ActionSService implements OnInit {
  ...
  getMembre():Observable<Membre[]>{
    return this.http.get<Membre[]>(URL2);
  }
  ...
  updateMembre(id: number, updatedMembre: any): Observable<Membre> {
    const url = `${URL2}/${id}`;
    return this.http.put<Membre>(url, updatedMembre);
  }
  addPersonne(m: Membre): Observable<Membre> {
    return this.http.post<Membre>(URL2, m);
  }
